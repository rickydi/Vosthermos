...
export async function GET(req, { params }) {
  ...
  if (!manager) return NextResponse.json({ error: "Non autorise" }, { status: 401 });
  ...
  if (!wo || wo.visibleAuClient === false) return NextResponse.json({ error: "Bon introuvable" }, { status: 404 });

  const managerClient = canAccessClient(manager, wo.clientId);
  if (!managerClient) return NextResponse.json({ error: "Acces refuse" }, { status: 403 });

  const documentMeta = getWorkOrderDocumentMeta(wo.statut, searchParams.get("documentType"));
  const permission = documentMeta.type === "quote" ? "view_quotes" : "view_invoices";
  if (!hasPermission(managerClient, permission)) {
