...
const problems = [
  { id: "thermos", label: "Foggy sealed units", icon: "fas fa-cloud", perWindow: 225, perPatio: 300, perEntry: 0, heatSaving: 40 },
  { id: "air", label: "Drafts / whistling", icon: "fas fa-wind", perWindow: 55, perPatio: 75, perEntry: 60, heatSaving: 35 },
  { id: "mecanique", label: "Stuck mechanisms", icon: "fas fa-cogs", perWindow: 80, perPatio: 150, perEntry: 100, heatSaving: 5 },
  { id: "calfeutrage", label: "Deteriorated caulking", icon: "fas fa-fill-drip", perWindow: 25, perPatio: 35, perEntry: 30, heatSaving: 25 },
  { id: "moustiquaire", label: "Damaged screens", icon: "fas fa-border-all", perWindow: 45, perPatio: 80, perEntry: 0, heatSaving: 0 },
  { id: "bois", label: "Rotted or damaged wood", icon: "fas fa-tree", perWindow: 120, perPatio: 0, perEntry: 200, heatSaving: 15 },
];
    ...
    <div className="pt-[80px] min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-br from-[var(--color-teal-dark)] to-[var(--color-teal)] py-12">
        <div className="max-w-[800px] mx-auto px-6 text-center">
          ...
          <p className="text-white/60 max-w-xl mx-auto">
            Find out how much you could save with the OPTI-FENETRE program
            vs. a complete replacement of your doors and windows.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-[900px] mx-auto px-6 py-12">
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden">
            ...
            <div className="grid sm:grid-cols-3 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <i className="fas fa-th-large text-[var(--color-teal)] mr-1"></i> Windows
                </label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <i className="fas fa-door-open text-[var(--color-teal)] mr-1"></i> Patio doors
                </label>
                <div className="flex items-center gap-3">
                  <input
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  <i className="fas fa-door-closed text-[var(--color-teal)] mr-1"></i> Entry doors
                </label>
                <div className="flex items-center gap-3">
                  <input
          ...
          {showResult && calc.totalOpenings > 0 && (
            <div className="p-8 bg-gray-50">
              <h2 className="text-lg font-bold text-gray-900 mb-8 text-center">Your results</h2>

              {/* Comparison cards */}
              <div className="grid md:grid-cols-2 gap-6 mb-8">
                {/* Replacement */}
                <div className="bg-white rounded-2xl p-6 border border-gray-200 text-center">
                  <p className="text-sm text-gray-500 mb-2">Full replacement</p>
                  <p className="text-4xl font-black text-gray-400 line-through">
                    {formatMoney(calc.replaceCost)}
                  </p>
                  <div className="mt-4 space-y-1 text-xs text-gray-400">
                    <p>{windows} windows x {formatMoney(REPLACEMENT_PRICES.window)}</p>
                    {patioDoors > 0 && <p>{patioDoors} patio door(s) x {formatMoney(REPLACEMENT_PRICES.patioDoor)}</p>}
                    {entryDoors > 0 && <p>{entryDoors} entry door(s) x {formatMoney(REPLACEMENT_PRICES.entryDoor)}</p>}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl p-6 border-2 border-[var(--color-teal)] text-center relative shadow-lg shadow-[var(--color-teal)]/10">
                  ...
                  <p className="text-sm text-gray-500 mb-2 mt-1">Complete restoration</p>
              ...
              <div className="bg-green-50 border border-green-200 rounded-2xl p-8 text-center mb-8">
                <p className="text-sm text-green-600 font-semibold mb-2">You save</p>
                <p className="text-5xl md:text-6xl font-black text-green-700 mb-2">
                  {formatMoney(calc.savings)}
                </p>
                <p className="text-green-600 font-bold text-xl">
                  That&apos;s {calc.savingsPercent}% in savings
                </p>
              </div>
              ...
              <div className="grid sm:grid-cols-3 gap-4 mb-8">
                <div className="bg-white rounded-xl p-5 text-center border border-gray-100">
                  ...
                  <p className="text-xs text-gray-500">Heating savings/year</p>
                </div>
                <div className="bg-white rounded-xl p-5 text-center border border-gray-100">
                  <i className="fas fa-clock text-blue-500 text-xl mb-2"></i>
                  <p className="text-2xl font-black text-gray-900">{calc.roi} years</p>
                ...
                <div className="bg-white rounded-xl p-5 text-center border border-gray-100">
                  ...
                  <p className="text-xs text-gray-500">Openings saved from replacement</p>
                </div>
              </div>

              {/* CTA */}
              <div className="bg-[var(--color-teal-dark)] rounded-2xl p-8 text-center text-white">
                <h3 className="text-xl font-bold mb-2">Ready to save {formatMoney(calc.savings)}?</h3>
                <p className="text-white/60 text-sm mb-6">Free quote with no obligation for your project</p>
              ...
              <div className="mt-8 grid sm:grid-cols-3 gap-3">
                <Link href="/en/opti-fenetre" className="flex items-center gap-3 p-4 bg-white rounded-xl border border-gray-100 hover:shadow-md transition-all group">
                  <i className="fas fa-star text-yellow-500"></i>
                  <span className="text-sm text-gray-700 group-hover:text-[var(--color-teal)]">OPTI-FENETRE Program</span>
                </Link>
                <Link href="/en/prix" className="flex items-center gap-3 p-4 bg-white rounded-xl border border-gray-100 hover:shadow-md transition-all group">
                  <i className="fas fa-dollar-sign text-green-600"></i>
                  <span className="text-sm text-gray-700 group-hover:text-[var(--color-teal)]">Full pricing guide</span>
                </Link>
                <Link href="/en/diagnostic" className="flex items-center gap-3 p-4 bg-white rounded-xl border border-gray-100 hover:shadow-md transition-all group">
                  <i className="fas fa-stethoscope text-purple-600"></i>
                  <span className="text-sm text-gray-700 group-hover:text-[var(--color-teal)]">Window diagnostic</span>
                </Link>
              </div>
        ...
        <div className="mt-12 max-w-[700px] mx-auto text-center">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Why restore rather than replace?</h2>
          <p className="text-gray-600 text-sm leading-relaxed mb-6">
            The Vosthermos OPTI-FENETRE program lets you restore your doors and windows —
            sealed units, hardware, weatherstripping, caulking and screens — in a single package.
            You get the same result as a full replacement for a fraction of the price.
          </p>
          <div className="flex flex-wrap justify-center gap-3 text-xs text-gray-400">
            <Link href="/en/problemes" className="hover:text-[var(--color-teal)]">50 documented problems</Link>
            <span>·</span>
            <Link href="/en/glossaire" className="hover:text-[var(--color-teal)]">Technical glossary</Link>
            <span>·</span>
