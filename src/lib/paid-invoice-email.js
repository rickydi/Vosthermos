...
function serializeMoneyFields(wo, { statut } = {}) {
  const serItem = (item) => ({
    ...item,
  ...
  return {
    ...wo,
    statut: statut || wo.statut || "invoiced",
...
function renderPaymentUpdateEmailHtml(wo, documentNumber, filename) {
  const summary = documentPaymentSummary(wo);
  const invoiceDate = formatLongDateFr(getDocumentDate(wo, "invoice"));
  const latestPayment = summary.payments.at(-1);
  const contactName = emailGreetingName(wo.client);
  const logoExists = fs.existsSync(LOGO_PATH);
  const friendly = isFriendlyBusinessClient(wo.client);
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Facture mise a jour ${escapeHtml(documentNumber)}</title>
</head>
<body style="margin:0;padding:0;background-color:#eef1f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#172033;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#eef1f5;padding:34px 16px;">
    <tr>
        ...
        <table role="presentation" width="640" cellpadding="0" cellspacing="0" border="0" style="max-width:640px;background-color:#ffffff;border-radius:14px;overflow:hidden;box-shadow:0 12px 34px rgba(23,32,51,0.12);">
            ...
            <td style="background-color:#b91c1c;padding:34px 40px 30px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  ...
                  <td align="right" valign="middle" style="color:#ffffff;">
                    <div style="font-size:11px;letter-spacing:2px;opacity:.82;font-weight:800;text-transform:uppercase;">Facture mise a jour</div>
                    <div style="font-size:25px;font-weight:900;margin-top:7px;line-height:1.1;">${escapeHtml(documentNumber)}</div>
                    <div style="font-size:12px;opacity:.88;margin-top:8px;">Date de facture : ${escapeHtml(invoiceDate)}</div>
                  </td>
                </tr>
          ...
          <tr>
            <td style="padding:36px 40px 18px;">
              ...
              <p style="margin:0 0 16px;font-size:15px;line-height:1.6;color:#374151;">
                ${friendly
                  ? `Nous confirmons la reception du paiement${latestPayment ? ` de <strong>${escapeHtml(formatMoneyCad(latestPayment.amount))}</strong>` : ""}. La facture PDF mise a jour est jointe a ce courriel.`
                  : `Nous confirmons la reception de votre paiement${latestPayment ? ` de <strong>${escapeHtml(formatMoneyCad(latestPayment.amount))}</strong>` : ""}. La facture PDF mise a jour est jointe a ce courriel.`}
              </p>
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;margin:22px 0;">
                <tr>
                  <td style="padding:18px 20px;">
                    <div style="font-size:11px;color:#8793a3;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;">Resume de la facture</div>
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:10px;">
                      <tr>
                        <td style="padding:6px 0;color:#667085;font-size:13px;">Total facture</td>
                        <td align="right" style="padding:6px 0;color:#172033;font-size:13px;font-weight:800;">${escapeHtml(formatMoneyCad(summary.total))}</td>
              ...
              <div style="background-color:#fff7ed;border:1px solid #fed7aa;border-radius:12px;padding:16px 18px;margin:0 0 24px;">
                <div style="font-size:13px;color:#9a3412;font-weight:900;margin-bottom:5px;">Piece jointe</div>
                <div style="font-size:14px;color:#4b5563;line-height:1.6;">Le PDF joint contient maintenant le paiement recu et le solde restant de la facture.</div>
              </div>
            </td>
          </tr>
          <tr>
...
function renderPaymentUpdateEmailText(wo, documentNumber, filename) {
  const summary = documentPaymentSummary(wo);
  const invoiceDate = formatLongDateFr(getDocumentDate(wo, "invoice"));
  const latestPayment = summary.payments.at(-1);
  const contactName = emailGreetingName(wo.client);
...
${friendly ? "J'espere que tu vas bien.\n\n" : ""}Nous confirmons la reception ${friendly ? "du paiement" : "de votre paiement"}${latestPayment ? ` de ${formatMoneyCad(latestPayment.amount)}` : ""}.

La facture PDF mise a jour est jointe a ce courriel.
...
Total facture: ${formatMoneyCad(summary.total)}
Paiements recus: ${formatMoneyCad(summary.paidTotal)}
Solde a payer: ${formatMoneyCad(summary.balanceDue)}

${summary.payments.length ? `Paiements:\n${paymentRowsText(summary.payments)}\n\n` : ""}Pour une question, repondez simplement a ce courriel.
...
export async function sendInvoicePaymentUpdateEmail(workOrder, { to } = {}) {
  ...
  const documentMeta = getWorkOrderDocumentMeta(serializedWo.statut, "invoice");
  ...
  await transporter.sendMail({
    ...
    subject: `Facture mise a jour ${documentNumber} - Vosthermos`,
    text: renderPaymentUpdateEmailText(serializedWo, documentNumber, filename),
    html: renderPaymentUpdateEmailHtml(serializedWo, documentNumber, filename),
    headers: {
...
export async function sendPaidInvoiceEmail(workOrder, { to } = {}) {
  ...
  const serializedWo = serializeMoneyFields(workOrder, { statut: "paid" });
